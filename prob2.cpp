...
int totalCost(int unit,int priceEachUnit = 100){
...
void YoungestEldeset(int age[],int size =3){
    int greatest = age[0];
    int youngest = age[0];
...
void attendenceCalculator(int classesHeld, int classesAttended){
    int percentAttendence = (classesAttended / classesHeld)*100;
    if(percentAttendence < 75){
        printf("Your attendence is : %d %",percentAttendence);
        printf("/n Sorry You arent allowed to sit in exam"); 
    }   
    else{
        printf("Your attendence is : %d %",percentAttendence);
        printf("/nCongo ! You are allowed to sit in exam ");
    }
}
